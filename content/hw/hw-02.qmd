...
Notes: - To insert an image in an RMarkdown document, you can use the syntax `![alt text](path/to/image.png)`. - The R/`ggplot2` code to create your visualization *cannot* already exist on the internet. (For example, choosing to recreate a plot from the [R Graph Gallery](https://r-graph-gallery.com/density_mirror_ggplot2.html) would *not* be an option b/c all the code is already there and you wouldn't learn as much.)
