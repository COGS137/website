...
And, in this lab we will work with the `tidyverse` package. So we need to load it:
...
::: aside
CSV is a delimited text file that uses a comma to separate values, though many implementations of CSV import/export tools allow other separators to be used as well. (Source: [Wikipedia](https://en.wikipedia.org/wiki/Comma-separated_values)
:::
...
-   Round `unemployment_rate`: We create a new variable with the `mutate` function. In this case, we're overwriting the existing `unemployment_rate` variable, by `round`ing it to `4` decimal places.For example, the call to `mutate` would be: `mutate(unemployment_rate = round(unemployment_rate, digits = 4))`
