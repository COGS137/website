...
9. Again using the `starwars` dataset, first, filter to only include those characters whose sex is not missing (`NA`). Then, using `ggplot2`, generate a bar plot that plots the `sex` along the x axis and the number of characters corresponding to that sex on the y-axis. The x-axis should be plotted by frequency of sex (the sex that has the most characters at left and that with the fewest to the right). Store this plot in the variable `p`. Note: In the `forcats` package there is a function `fct_infreq`. We did not discuss this in class, but we did a similar example. Check out the documentation using `?fct_infreq` to learn how to use it for this question.
