...
In this lab we will work with the `tidyverse`, `broom`, `infer`, and `openintro` packages. We can load them with the following:
...
7. Considering the results from the model you just fit and using a backward elimination strategy, remove any variables from the `m_multiple` model fit previously to identify the most parsimonious model. Store this in `m_resume`. Utilize AIC (Akaike information criterion) for model comparison.
...
8. Using your results from `m_resume`, estimate the probability of receiving a callback for a job in Chicago where the candidate lists 14 years experience, no honors, no military experience, includes an email address, and has a first name that implies they are a White male. How do these results differ from someone who has a first name that implies they are a Black male? 