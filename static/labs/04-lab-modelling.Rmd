...
In this lab we will work with the `tidyverse` and `broom` packages. Be sure to load them in before continuing with the lab.
...
1.  Create a new variable called `bty_avg` that is the average attractiveness
    ...
    `rowwise` function. Since `rowwise` is new to you, incomplete code is given below 
    to guide you in the right direction, however you will need to fill in the blanks.
